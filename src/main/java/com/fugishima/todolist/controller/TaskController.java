package com.fugishima.todolist.controller;
...
import com.fugishima.todolist.Utils.Utils;
import com.fugishima.todolist.model.TaskModel;
import com.fugishima.todolist.repository.ITaskRepository;
    ...
    public ResponseEntity<?> create(@RequestBody TaskModel taskModel, HttpServletRequest request) {
        ...
        if (currentDateTime.isAfter(taskModel.getStartAt()) || currentDateTime.isAfter(taskModel.getEndAt())) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body("The start date and/or end date must be after than current date time");
        }

        if (taskModel.getStartAt().isAfter(taskModel.getEndAt())) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body("The end date must be after than start date");
        }
